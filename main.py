...
'''def saveVideoFrame():
    frameDataDictionary = {}
    poseData = {}
    videoPath = "Videos/DanceCoverJennie.MOV"
    cap = cv2.VideoCapture(videoPath)

    i = 0
    while cap.isOpened():
        flag, frame = cap.read()
        # if the frame is read correctly, flag is true
        ...
        print(i)
        print(flag)
        rob = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        cv2.imshow('frame', rob)
        if cv2.waitKey(1) == ord('q'):
            break
        if i % 4 == 0:
            pose = mp_pose.Pose()
            results = pose.process(rob)
            # with mp_pose.Pose(static_image_mode=True, min_detection_confidence=0.5, model_complexity=2) as pose:
            #     results = pose.process(frame)

            print(f'frame{i}', results.pose_landmarks)
            try:
                danceLandmarks = {"leftShoulder": [results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.LEFT_SHOULDER].x,
                                                   results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.LEFT_SHOULDER].y],
                                  "rightShoulder": [results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_SHOULDER].x,
                                                    results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_SHOULDER].y],
                                  "leftElbow": [results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.LEFT_ELBOW].x,
                                                results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.LEFT_ELBOW].y],
                                  "rightElbow": [results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_ELBOW].x,
                                                 results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_ELBOW].y],
                                  "rightWrist": [results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_WRIST].x,
                                                 results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_WRIST].y],
                                  "leftWrist:": [results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.LEFT_WRIST].x,
                                                 results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.LEFT_WRIST].y],
                                  "leftHip": [results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.LEFT_HIP].x,
                                              results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.LEFT_HIP].y],
                                  "rightHip": [results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_HIP].x,
                                               results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_HIP].y],
                                  "leftKnee": [results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.LEFT_KNEE].x,
                                               results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.LEFT_KNEE].y],
                                  "rightKnee": [results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_KNEE].x,
                                                results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_KNEE].y],
                                  "leftAnkle": [results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.LEFT_ANKLE].x,
                                                results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.LEFT_ANKLE].y],
                                  "rightAnkle": [results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_ANKLE].x,
                                                 results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_ANKLE].y],
                                  "nose": [results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.NOSE].x,
                                            results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.NOSE].y],
                                  "leftHeel": [
                                      results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.LEFT_HEEL].x,
                                      results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.LEFT_HEEL].y],
                                  "rightHeel": [
                                      results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_HEEL].x,
                                      results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_HEEL].y],
                                  "leftToe": [
                                      results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.LEFT_FOOT_INDEX].x,
                                      results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.LEFT_FOOT_INDEX].y],
                                  "rightToe": [
                                      results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_FOOT_INDEX].x,
                                      results.pose_world_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_FOOT_INDEX].y],
                                  }
            except:
                print("missing joint")
                pass
            frameDataDictionary[f'frames{i}'] = danceLandmarks
            poseData[f'frame{i}'] = danceLandmarks
            os.chdir('keyframes')
            cv2.imwrite(f'frames{i}.jpg', frame)
            os.chdir('..')
        outfile = open("data.json", 'w')
    jsonString = json.dumps(frameDataDictionary, indent = 4)
    outfile.write(jsonString)
    convertVidToAudio_ffmpeg(videoPath, outputExt="mp3")
...
def convertVidToAudio_ffmpeg(videoFile, outputExt = "mp3"):
    filename, ext = os.path.splitext(videoFile)
    outputAudioPath = f"{filename}.{outputExt}"
    os.popen(f'ffmpeg -y -i {videoFile} {outputAudioPath}').read()
    return outputAudioPath

#convertVidToAudio_ffmpeg("Videos/DanceCoverJennie.MOV", outputExt = "mp3")'''